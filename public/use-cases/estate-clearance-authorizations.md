...
An independent witness layer may apply — see [Witnessing Third Parties](../../docs/WITNESSING-THIRD-PARTIES.md) for the full mechanism (what a witness receives, non-repudiation, optional public-blockchain rollups). For this use case: a witness would receive the court or executor's hashes and status changes plus structured metadata (property address, deceased name, probate reference, executor name, clearance firm name, license number, authorized dates) — never plaintext estate details — providing non-repudiation of the estate clearance authorization.
