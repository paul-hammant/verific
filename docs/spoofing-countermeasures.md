...
- **Interim client guards — each client's own heuristic.** The protocol should not mandate a specific test; clients treat issuer-supplied rules as untrusted input and guard as they see fit. A rule-count cap is the obvious example (a legitimate folding meta needs a handful; a meta shipping many is meaning-altering or a ReDoS payload — refuse it and flag it loudly). Others: per-rule execution timeouts, a total-normalization-time budget, rejecting backreferences/nested quantifiers, bounding replacement length. Each phone app, extension, or browser-native client makes its own call; none should silently apply arbitrary regex from the party being verified.
