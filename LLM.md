...
**Optional: `appendToHashResourceName`** (legacy alias: `appendToHashFileName`) — Specifies what to append to the hash when building the lookup URL. If omitted, the app requests the bare hash path (e.g., `https://example.com/c/{hash}`). GitHub Pages users typically use `{hash}/index.html` directories, which GitHub serves at the bare path via 302 — so no suffix is needed. Issuers on other infrastructure can set `".json"` to produce `https://example.com/c/{hash}.json`, or any other suffix appropriate to their server configuration. (This field was previously documented here as `hashSuffix`, which no client has ever read.)

**Optional: `hashesHostedAt`** — Absolute base URL where the hash files actually live, e.g. `"https://provider.example.com/acme"`. Hash lookups go there instead of to the `verify:` line's path; everything else, including `verification-meta.json` itself, is still fetched from the issuer's own domain.

This is a **hosting hint, not a delegation of authority.** The domain shown to the verifier is always the one the document named on its `verify:` line. An issuer may put its hash files behind a provider for a couple of years and then bring them in-house, and neither the verification nor what the reader sees should change — where the bytes live is infrastructure, like a CDN. Clients therefore derive the displayed authority (and its PSL-emphasised registrable domain) from the base URL, never from the verification URL. `verifyHash()` takes the authority domain as a required argument and throws rather than guessing it.

**Optional: `authorityBasis`** — One short line (under 80 chars) stating what kind of authority backs this issuer's verifications, shown to the verifier so they can tell a sovereign attestation from a personal opinion. Personal attestations must include "Individual" or "personal". Displayed as "Authority claimed" — claimed, because with no `authorizedBy` nobody has endorsed the wording. See docs/authorityBasis_and_authorizedBy_patterns.md.
...
- Public-suffix operator disclaimer (PROPOSAL, unimplemented): lets the operator of a multi-tenant namespace (GitHub for `github.io`, Automattic for `wordpress.com`) publish `verification-meta.json` at the suffix apex disclaiming tenants trading on its name — the mirror image of `authorizedBy`. Exactly one location, no `www.` retry (on a public suffix `www.` is a *tenant*, not the operator, and there is no authoring variance to absorb since the suffix comes from the PSL), so absence unambiguously means the operator has said nothing. Disclaim-only, not endorse (see docs/public-suffix-operator-disclaimer.md)
...
- xz-utils case study (two-repo, with ISNAD): what a Live Verify release claim would have said on each side of CVE-2024-3094 — correctly `verified` for the backdoored release (authentic ≠ safe), then `REVOKED`; the SIDEWAYS endorser chain for open source (build attestation + independent rebuilders) since there is no regulator or sovereign root — amber-unanchored by design, with a route to green via CRA/CISA/BSI recognising rebuilder networks; endorser-side RESTRICT is essential because the issuer may be the compromised party; three spec gaps (multi-valued Attested-By, endorser RESTRICT, release scope vocabulary); a revocation is an ISNAD evidence record; and the RETURN direction — a portal's own package-approver BOT may consume a grader's evidence (ISNAD is one example, not required) and GATE issuance, withholding `OK` and publishing `PENDING` (proposed new amber status, never auto-promotes, distinct from 404) until checks pass, so 5.6.0 never reaches `OK` in the first place. The verdict→status mapping is the PORTAL's policy; Live Verify keeps its statuses few/plain/human-first (`OK`/`PENDING`/`REVOKED`/`RESTRICTED`), machines follow `More:` for detail. Live Verify and any grader never touch directly (see docs/xz-release-attestation-case-study.md, and ISNAD's docs/case-study-xz-sleeper-narrator.md)
